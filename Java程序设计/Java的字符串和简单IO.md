...
先说下`obj==null`。我们已经了解过Java中对象即指针，输出一个没有被构造过的指针指向的内容是危险的，所以需要先判断这个指针是不是`null`，所以一个没有被构造的对象是可以转换成`String`的（但是不能直接当作参数输入`String.valueOf(...)`，Java特性），其得到的内容是`null`。而`obj`不是`null`就会调用`obj`的`.toString()`函数
